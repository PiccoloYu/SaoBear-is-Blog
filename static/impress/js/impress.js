...
(function (document, window) {
    ...
    var impress = window.impress = function (rootId) {
        ...
        //初始化所有步骤。
        //读取data-*属性，存储在内部stepsdata中，用css进行渲染。
        ...
        //`转到“api”函数，该函数移动到指定为“el”参数的步骤（按索引、id或元素）。
        //` duration`可选地作为第二个参数，是css中的转换持续时间。
        //` reason`是字符串“next”、“prev”或“goto”（默认值），将提供给
        //presteppleave插件。
        //` origevent`可能包含导致调用goto的事件，例如按键事件
        var goto = function (el, duration, reason, origEvent) {
            ...
            //对每个转换重新执行initallsteps。这允许编辑步骤属性
            //动态地，例如更改它们的坐标，甚至删除或添加步骤，并且
            //调用goto（）时应用该更改。
            initAllSteps();
            ...
            //有时可以通过一些键盘操作触发对第一个链接的关注。
            //browser在这种情况下试图滚动页面以使该元素可见
            //（即使身体溢出被设置为隐藏），也会破坏我们的小心定位。
            //因此，作为一个糟糕的（和懒惰的）解决方案，我们将使页面滚动回顶部
            //每当选择幻灯片时
            window.scrollTo(0, 0);
            ...
            //这里是触发“impress:stepEnter”事件的地方。
            //我们只需设置一个超时值，在转换持续时间（和可能的延迟）内触发它
            //考虑到。

            //我真的很想让它更优雅。“transitionEnd”事件似乎
            //是最好的方法，但事实上我在两个独立的
            //elements和“transitionEnd”事件仅在
            //transition（值的更改）导致了一些错误，并使代码真正
            //很复杂，因为我必须分别处理所有条件。它仍然
            //在完全没有转换的情况下需要“setTimeout”回退。
            //所以我决定让代码更简单，而不是使用闪亮的new
            //` transitionEnd`。

            //如果你想学习一些有趣的东西，看看如何使用'transitionEnd'`
            //返回impress.js的0.5.2版本：
        ...
        //`prev` API函数转到上一步（按文档顺序）
        //`event`是可选的，可能包含导致需要调用prev（）的事件
        ...
        let imglist = ['timg', 'timg2'];
        //`next`API函数进入下一步（按文档顺序）向下
        //`event`是可选的，可能包含导致需要调用next（）的事件
        var next = function (origEvent) {
            var next = steps.indexOf(activeStep) + 1;
            /*let supported = document.querySelector('.background');
            supported.style.background = `url('./img/${next % 2 == 0 ? imglist[0] : imglist[1]}.jpg')`;
            supported.style.backgroundRepeat = "no-repeat";
            supported.style.backgroundSize = "cover";
            supported.style.backgroundPositionX = "center";
            supported.style.backgroundPositionY = "center";
            supported.style.opacity = 1;*/
        ...
        //设置滑动动画。
        //pct是介于-1.0和+1.0之间的值，指定当前长度
        //刷卡次数。
        //如果PCT为负，则向下一步滑动，如果为正，
        //朝着prev（）步骤。
        //注意，像goto这样的pre-steppleave插件可能会破坏
        //next（）和prev（）步骤，因此我们需要触发pre-stepLeave事件
        //这里，即使刷卡不能保证转换
        //真的发生了。
        //调用swipe（），其值为pct，本身不会导致
        //transition要发生，这只是设置滑动的动画。一旦
        //transition是提交的-例如在touchend事件中-调用方是
        //还负责根据需要调用prev（）/next（）。
        //note:目前，swipe插件可以使用这个函数
        //是与此对应的UI）。它是一个半内部api，故意不
        //文档记录在documentation.md中。
...
(function (document, window) {
    ...
    //core初始化
    //library工厂（gc（rootid））在impress（rootid）.init（）的开头调用
    //为了teardown（）的目的，我们可以利用这个机会保存状态
    //dom中一些原始状态的东西，在impress（）.init（）做任何事情之前。
    //注意：这些值也可以由impress.js core中的代码记录为
    //，但为了不偏离上游，我添加了
    //它们在这里而不是核心本身。
...
(function (document) {
    ...
    //如果在演示文稿根目录中定义了默认的自动播放时间，或者
    //在此步骤中，设置超时。
    var reloadTimeout = function (event) {
        var step = event.target;
        currentStepTimeout = util.toNumber(step.dataset.autoplay, autoplayDefault);
        if (status === "paused") {
            setAutoplayTimeout(1000);
        } else {
            setAutoplayTimeout(0); //设置自动切换
        }
    };
    ...
    */
    var setAutoplayTimeout = function (timeout) {
        ...
        if (timeout > 0) {
            timeoutHandle = setTimeout(function () { api.next(); }, timeout === 0 ? 0 : 1000);//timeout * 1000 timeout === 0 ? 0 : 3000
        }
        setButtonText();
    };
    ...
    var addToolbarButton = function (toolbar) {
        var html = '<button id="impress-autoplay-playpause" class="but playpause" ' + // jshint ignore:line
...
(function (document) {
    ...
    var addNavigationControls = function (event) {
        ...
        var prevHtml = `<button id="impress-navigation-ui-prev"  title="Previous"
            class="impress-navigation-ui but Switch">&lt;</button>`;
        var selectHtml = '<select id="impress-navigation-ui-select"  title="Go to" ' +
            'class="impress-navigation-ui select">' + '\n' +
            selectOptionsHtml() +
            '</select>';
        ...
        body.insertAdjacentHTML('afterbegin', prevHtml);
        body.insertAdjacentHTML('afterbegin', nextHtml);*/
...
(function (document) {
    "use strict";
    var toolbar = document.getElementById("impress-toolbar");// document.getElementById("impress-toolbar");
    var groups = [];
    ...
    if (toolbar) {
         ...
         */
        toolbar.addEventListener("impress:toolbar:appendChild", function (e) {//添加按钮
